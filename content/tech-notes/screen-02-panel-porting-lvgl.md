...
draft = false
weight = 2
tags = ['BK7258 平台', '屏幕驱动', 'LVGL', '硬件适配', '调试排障']
series = 'LVGL 屏幕适配'
+++

整机都在假装这块屏不存在——Post、AP、ATE 谁也不真正刷屏，因为产品一直跑在「无屏」路径上。等到真要把 ST7567（128×48 正显、4 线 SPI、背光落在 GPIO24）点亮，才发现麻烦横跨三个仓，还顺手牵出告警灯测项与硬解码显示通路两条支线。
