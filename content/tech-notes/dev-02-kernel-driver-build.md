...
draft = false
weight = 7
tags = ['嵌入式', '内核驱动', '编译构建', 'BK7258 平台', '调试排障']
series = '开发环境与内核构建'
+++

代码明明改了，内核里却找不到这个驱动；换到容器里编译，又被 CMake 版本过低按在地上。在 BK7258 与 RK3506 两套 SDK 之间来回切的日子里，最消耗人的从来不是写驱动，而是搞清自己此刻到底在编哪一套。下面把编译、驱动集成和调试记录一并记下。
