...
draft = false
weight = 4
tags = ['固件开发', 'ATE 产测', '网络', '产测协议', '调试排障']
series = 'ATE 产测实战'
+++

最气人的从来不是失败，是程序说自己成功。`test_wifi_tx` 明明回了通过，IQ2010 上却只有约 -20.4 dBm——仪表不认账。折腾一圈才发现问题出在探头上，对准 ANT-221 才复测过关。同机的 BLE 也不省心，早期建连要等 65 s，久到足够泡一杯茶。下面把这几条链路一条条摊开。
