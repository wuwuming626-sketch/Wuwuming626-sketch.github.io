...
draft = false
weight = 2
tags = ['嵌入式', '低功耗', '网络', '调试排障', '内核驱动']
+++

A330i 版本 2.20.3 静置久了会偶现黑屏（T118735）。现场没留下别的证据，只有一个黑屏状态下导出的一键日志包——像案发现场唯一没被踩坏的脚印。顺着它往下走，又撞上唤醒要等 4～5 s、eth0 DMA 复位失败，以及重启后 eth0 与 wlan0 一齐 DOWN 无 IP。下面把这几条链路的排查过程一条条记下来。
